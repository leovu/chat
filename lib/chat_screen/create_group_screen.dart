...
class _CreateGroupScreenState extends AppLifeCycle<CreateGroupScreen> {
  ...
  void _applyFilter() {
    final val = _controllerSearch.value.text.toLowerCase().removeAccents();
    if (val != '') {
      final visible = Contacts();
      visible.limit = contactsListData?.limit;
      visible.search = contactsListData?.search;
      visible.users = (contactsListData?.users ?? []).where((element) {
        try {
          final fullName = '${element.firstName} ${element.lastName} '
              '${element.username}'
              .toLowerCase()
              .removeAccents();
          return fullName.contains(val);
        } catch (_) {
          return false;
        }
      }).toList();
      ...
      visible.search = contactsListData?.search;
      try {
        visible.users = <People>[...contactsListData!.users!.toList()];
      } catch (_) {}
      contactsListVisible = visible;
    }
  }
            ...
            body: SafeArea(
              child: Column(
                children: [
                  ...
                  contactsListVisible != null && _selectedIds.isNotEmpty
                      ? Padding(
                            ...
                            child: MaterialButton(
                              ...
                              onPressed: () async {
                                ...
                                if (people.isEmpty) {
                                  ...
                                  r.Rooms? rooms =
                                      await ChatConnection.createGroup(
